...
On Windows, runtime caches and non-secret connection settings live under `%LOCALAPPDATA%\QuantDinger\Connector`. Credentials use the `QuantDinger Connector` service in the OS vault. Older credential revisions and runtime directories may remain after upgrades or plugin removal.
...
The tests use local HTTP fixtures and generated dummy tokens. Never replace those values with production secrets. The repository currently ships a Windows x64 runtime only.
